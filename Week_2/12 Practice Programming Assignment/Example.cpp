...
int main() 
{
	...
	for (int i = 0; i < q; ++i) 
	{
		...
		if (operation_code == "RENAME") 
		{
			...
			if (old_country_name == new_country_name || country_to_capital.count
			(old_country_name) == 0 || country_to_capital.count(new_country_name) == 1) 
			{
			...
			else 
			{
				...
				country_to_capital[old_country_name] <<
